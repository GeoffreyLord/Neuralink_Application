    ...
    for l in range(len(model.layers)):
        for n in range(len(model.layers[l].neurons)):
            for w in range(len(model.layers[l].neurons[n].w)):
                model.layers[l].neurons[n].w[w].data = torch.tensor([model_params[counter]], dtype=torch.double)
                counter += 1

    """ Add Bias Values """
    for l in range(len(model.layers)):
        for n in range(len(model.layers[l].neurons)):
            model.layers[l].neurons[n].b.data = torch.tensor([model_params[counter]], dtype=torch.double)
            counter += 1
...
def initalize(args):    
    ...
    #TODO: I do not know if this is needed. There must be somewhere else where this is being checked. 
    if args.evaluate:
